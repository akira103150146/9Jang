...
md.renderer.rules.fence = (tokens, idx, options, env, self) => {
  ...
  const embedPos = env?.__embedPos?.[idx] // 0-based char index in original markdown (processed)
  const posAttr = Number.isFinite(embedPos) ? ` data-source-pos="${embedPos}"` : ''
  const embedLen = env?.__embedLen?.[idx]
...
export function useMarkdownRenderer() {
  ...
  const renderMarkdownWithLatex = (text) => {
    ...
    const fixLeadingDoubledDollarBeforeThereforeInline = (s) => {
      return String(s)
        .split('\n')
        .map((line) => {
          // 只修正：行首 $$∴$...$（允許空白）
    ...
    const computeLineNumberFromIndex = (source, idx) => {
    ...
    // 對 diagram2d/diagram3d/circuit fenced block 建立 idx -> pos 對照（供 fence rule 使用）
    // 這裡用 processed（尚未 md.render）版本來找 offset，因為最後用的是 md.render(processed)
    const embedPosMap = {}
    const fenceRegex = /```(diagram2d|diagram3d|circuit)[^\n]*\n[\s\S]*?\n```/gi
    let m
    while ((m = fenceRegex.exec(text)) !== null) {
      // 先存起來，之後在 env 裡用 token index 對應（順序對應 markdown-it tokens 的出現順序）
      // 實作上我們會在 render 時按 tokens idx 增加一個 counter 來對應，故這裡先放陣列
      // （先掛在 env.__embedList）
    ...
    let processed = text.replace(/\$\$([\s\S]*?)\$\$/g, (match, formula, offset) => {
    ...
    const env = { __embedPos: {}, __embedLen: {} }
    let embedCursor = 0
    ...
    html = html.replace(/<span data-latex-block="(\d+)" data-source-line="(\d+)" data-source-pos="(\d+)" data-source-len="(\d+)" data-math-delim="(\$\$)" data-math-raw="([^"]*)"><\/span>/g, (m, idxStr, lineStr, posStr, lenStr, delim, raw) => {
    ...
    html = html.replace(/<span data-latex-inline="(\d+)" data-source-line="(\d+)" data-source-pos="(\d+)" data-source-len="(\d+)" data-math-delim="(\$)" data-math-raw="([^"]*)"><\/span>/g, (m, idxStr, lineStr, posStr, lenStr, delim, raw) => {
      const i = Number(idxStr)
      const line = Number(lineStr)
      const pos = Number(posStr)
      const len = Number(lenStr)
