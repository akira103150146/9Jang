 ...
 * 編輯器貼上選項
 */
export interface EditorPasteOptions {
  editor?: Ref<Editor | null> | (() => Editor | null) | Editor | null
  imageMappings?: Ref<ImageMappings> | (() => ImageMappings) | ImageMappings | null
}
...
export function useEditorPaste(options: EditorPasteOptions = {}) {
  ...
  const getEditor = (): Editor | null => {
    ...
    if (editor && 'value' in editor) return (editor as Ref<Editor | null>).value
   ...
   */
  const getImageMappings = (): ImageMappings | null => {
    if (typeof imageMappings === 'function') return imageMappings()
    if (imageMappings && 'value' in imageMappings)
      return (imageMappings as Ref<ImageMappings>).value
    return (imageMappings as ImageMappings | null) || null
  }
   ...
   * 處理文字貼上
   */
  const handleTextPaste = async (event: ClipboardEvent, text: string): Promise<boolean> => {
    ...
    try {
      ...
      const nodes = createNodesFromTokens(tokens, editorInstance, mappings || undefined)
