...
export function usePrintStyleExtraction() {
   ...
   */
  const removeScopedAttributes = (element: HTMLElement): void => {
   ...
   */
  const extractComputedStylesFromEditor = (editorContainer: EditorContainer | null): string | null => {
    try {
      ...
      if (sqrtElements.length > 0) {
        const sqrtSample = sqrtElements[0] as HTMLElement
        const sqrtComputed = window.getComputedStyle(sqrtSample)

        // 提取根號內部的 vlist-t 樣式
        const vlistT = sqrtSample.querySelector('.vlist-t') as HTMLElement | null
        if (vlistT) {
          const vlistTComputed = window.getComputedStyle(vlistT)

          // 提取根號符號容器樣式
          const sqrtSign = sqrtSample.querySelector('.sqrt-sign') as HTMLElement | null
...
.katex .sqrt .sqrt-sign > span {
  ...
  overflow: hidden !important;
      ...
      if (msupsubElements.length > 0) {
        const msupsubSample = msupsubElements[0] as HTMLElement
        const msupsubComputed = window.getComputedStyle(msupsubSample)

        const vlistT = msupsubSample.querySelector('.vlist-t') as HTMLElement | null
        if (vlistT) {
          const vlistTComputed = window.getComputedStyle(vlistT)
...
.katex .msupsub .vlist-t {
   ...
   * 應用列印樣式到答案/詳解區域
   */
  const applyPrintStylesToSection = (section: HTMLElement): void => {
   ...
   * 應用列印樣式到標籤
   */
  const applyPrintStylesToLabel = (label: HTMLElement): void => {
   ...
   * 應用列印樣式到內容
   */
  const applyPrintStylesToContent = (content: HTMLElement): void => {
