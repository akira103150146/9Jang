  ...
  <div class="space-y-6">
    ...
    >
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <p class="text-sm font-semibold text-slate-500 dark:text-slate-400">課程模組</p>
          <h2 class="text-2xl font-bold text-slate-900 dark:text-white">課程與時段管理</h2>
          <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">掌握每堂課的授課老師、費用與狀態</p>
        </div>
        <router-link
          to="/courses/add"
    ...
    <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400">Course #{{ course.course_id }}</p>
            <h3 class="mt-1 text-lg font-semibold text-slate-900 dark:text-white">{{ course.course_name }}</h3>
          </div>
          <span
        ...
        <dl class="mt-4 space-y-2 text-sm text-slate-600 dark:text-slate-300">
          <div class="flex justify-between">
            <dt class="font-medium text-slate-500 dark:text-slate-400">授課老師</dt>
            <dd>{{ course.teacher_name || '未指派' }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="font-medium text-slate-500 dark:text-slate-400">上課時間</dt>
            <dd>{{ getDayDisplay(course.day_of_week) }} ・ {{ formatTime(course.start_time) }} - {{ formatTime(course.end_time) }}</dd>
          </div>
          <div class="flex justify-between">
            <dt class="font-medium text-slate-500 dark:text-slate-400">每堂費用</dt>
            <dd class="font-semibold text-slate-900 dark:text-white">${{ course.fee_per_session }}</dd>
          </div>
        </dl>
    ...
    <div v-if="!loading && courses.length === 0" class="text-center py-12">
      <p class="text-slate-500 dark:text-slate-400">目前沒有課程資料。</p>
    </div>
  </div>
</template>
