...
const addLine = () => {
  ...
  const points = board.objectsList.filter(obj => obj.elType === 'point')
  if (points.length < 2) {
    alert('請先添加至少兩個點')
    return
  }
  ...
  elements.value.push({ type: 'line', id: line.id, data: { point1: points[0].id, point2: points[1].id } })
  updateDiagramData()
}
...
const addPolygon = () => {
  ...
  elements.value.push({ type: 'polygon', id: polygon.id, data: { vertices: points.slice(0, 3).map(p => p.id) } })
  updateDiagramData()
}
...
const loadDiagramData = (data) => {
  ...
  data.elements.forEach(element => {
    switch (element.type) {
      case 'point':
        board.create('point', [element.data.x, element.data.y], { name: element.name })
        break
      case 'function':
...
const updateDiagramData = () => {
  ...
  const diagramData = {
    elements: elements.value.map(el => {
      const obj = board.objectsList.find(o => o.id === el.id)
      if (!obj) return el
      
      switch (el.type) {
        case 'point':
          return { type: 'point', data: { x: obj.X(), y: obj.Y() } }
        case 'function':
  ...
  let backupImage = ''
  try {
    if (board && board.renderer && board.renderer.svgRoot) {
      const svg = board.renderer.svgRoot.cloneNode(true)
