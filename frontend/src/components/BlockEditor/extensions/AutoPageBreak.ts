 ...
 * Auto Page Break 擴展
 */
export const AutoPageBreak = Extension.create<AutoPageBreakOptions, ExtensionConfig>({
  name: 'autoPageBreak',

  addOptions(): AutoPageBreakOptions {
    return {
    ...
    return {
      Enter: ({ editor }) => {
        ...
        try {
          ...
          let currentNode = domAtPos.node as HTMLElement
          ...
          // 取得游標提示方塊的高度
          const cursorIndicator = document.querySelector('.cursor-indicator') as HTMLElement | null
          ...
          // 計算當前在第幾頁
          const currentPage = Math.floor(adjustedBottom / (this.options.pageHeightPx || 971))

          // 計算當前頁的剩餘空間
          const nextPageBoundary = (currentPage + 1) * (this.options.pageHeightPx || 971)
          ...
          if (remainingSpace < 100) {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                // 將游標移到分頁符號後的新段落中
                editor.chain().focus(cursorPosAfterPageBreak).run()

                // 滾動到新位置
                setTimeout(() => {
                  const newPos = editor.state.selection.$from.pos
                  const domAtNewPos = editor.view.domAtPos(newPos)
                  let newNode = domAtNewPos.node as HTMLElement
