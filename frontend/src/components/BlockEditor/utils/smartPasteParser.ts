 ...
 * LaTeX Token
 */
export interface LatexToken extends BaseToken {
  type: 'latex'
  content: string
  displayMode: boolean
}
 ...
 * Token 聯合類型
 */
export type Token = LatexToken | HeadingToken | ListToken | BlockquoteToken | ImageToken | ParagraphToken

/**
 * 跨行 LaTeX 區塊
 */
interface BlockLatexBlock {
  startLine: number
  endLine: number
  formula: string
  startPos: number
 ...
 * 解析智能貼上內容
 * @param text - 貼上的純文字內容
 * @returns tokens 陣列，每個 token 包含 type 和 content
 */
export function parseSmartPaste(text: string): Token[] {
  if (!text || !text.trim()) {
    return []
  }
  ...
  const blockLatexBlocks: BlockLatexBlock[] = []
  ...
  while (i < lines.length) {
    ...
    const blockLatexBlock = blockLatexBlocks.find((block) => i >= block.startLine && i <= block.endLine)
    ...
    if (blockquoteMatch) {
      ...
      if (blockquoteLines.length > 0) {
        ...
        while ((imageMatch = imageRegex.exec(blockquoteText)) !== null) {
          images.push({
            alt: imageMatch[1],
            path: imageMatch[2],
    ...
    while (i < lines.length) {
      ...
      // 檢查當前行是否在跨行 LaTeX 區塊內
      const isInBlockLatex = blockLatexBlocks.some((block) => i >= block.startLine && i <= block.endLine)
      if (isInBlockLatex) {
...
/**
 * 解析行內 LaTeX 公式
 * @param text - 包含行內 LaTeX 的文字
 * @returns 文字片段陣列，每個片段標記是否為 LaTeX
 */
export function parseInlineLatex(text: string): TextPart[] {
